...
* An `object` called `YourOp` extending `common_etl.operator.dataset_series.DatasetSeriesContractOp` should be placed in the `nu.data.<country>.dataset_series` [package](https://github.com/nubank/itaipu/tree/master/src/main/scala/nu/data/br/dataset_series) (this link is only for BR series, change the url to the country you need). See [Code Generation](#code-generation) below for best practices on generating it from scratch.
